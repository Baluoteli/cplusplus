...
BOOL CYUVTrans::RGB32ToI420(const LPBYTE lpBGR32Buffer, LPBYTE lpYUVBuffer, SIZE_T nYUVBufSize, int nWidth, int nHeight)
{
	if (nYUVBufSize < nWidth*nHeight * 3 / 2)
		return FALSE;
	...
	for (int j = 0; j < nHeight; j++) {
// 		LPBYTE lpBufRGB = lpBGR32Buffer + nWidth * (nHeight - j) * 4 - 1;
// 
// 		for (int i = 0; i < nWidth; i++) {
// 			(lpBufRGB--);
// 			BYTE b = *(lpBufRGB--);
...
// 			BYTE r = *(lpBufRGB--);
		...
		LPBYTE lpBufRGB = lpBGR32Buffer + nWidth * (nHeight - 1 - j) * 4;
